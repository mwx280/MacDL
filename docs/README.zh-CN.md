...
- **系统通知**：开始 / 完成 / 失败可分别开关，失败通知带「重新下载」按钮；点击下载通知会打开主窗口。
...
| `Chunk.swift` | 一个字节区间及其进度；可 `Codable`，重启后状态能恢复。同时拥有块状态重建：紧凑续传格式（`CompletedRange`/`PartialChunk`）及其重建/补齐/合并逻辑。 |
...
单个下载和全局都设了并发上限，超出上限的任务进等待队列。全局上限由引擎的 `DownloadScheduler` 持有：新下载在有槽位时立即开始，否则按 FIFO 排队；当某任务结束（或上限调大）时，经晋升回调自动开始下一个等待中的下载。

**持久化**

分块进度不存全量分块数组，而是存合并后的连续已完成区间，以及各个分块的续传偏移。这样一份 100 GB 的文件下载完，落盘也只有几个区间条目，而不是约 40 万个分块。压缩与重建是引擎的块语义（`Chunk.completedRanges` / `Chunk.rebuildChunks`），App 模型只负责存取与展示。进度每 5 秒保存一次，退出时立即落盘。
...
- **引擎（172 个）**：Swift Testing + 假 `URLProtocol`，不发真实网络请求。覆盖分块完整性、暂停/续传、限速、退避、单线程回退、Range 边界、多源故障转移、动态分块、块状态重建、跨下载调度、优先级调度、网络可达性 hold/恢复、stall 检测与重试、自适应策略交互、校验和、Metalink 解析、源调度和 FTP。
- **应用（181 个）**：XCTest + 假引擎，不碰真实磁盘和通知中心。覆盖下载生命周期、优先流程、重复下载策略、持久化往返、Metalink 失败处理、更新状态机和本地化。
